...
function initGame(p1Human, p2Human, keepScore = false) {
    isVsComp = !p2Human;
...
function handleDraw() {
    ...
    if (boneyard.pieces.length > 0) {
        ...
        // If the drawn tile is playable, highlight it
        if (tile[0] === head || tile[1] === head || tile[0] === tail || tile[1] === tail) {
            ...
            // Show the draw button again if the tile isn't playable
            document.getElementById('drawButton').style.display = 'block';
        }
    } else {
        logMove(`${player.name} passes - boneyard is empty`);
        if (lastTurnWasPass) {
            gameEnd();
            return;
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('twoPlayerBtn').onclick = () => {
        initGame(true, true, false); // false = don't keep score for new game
    };

    document.getElementById('vsCompBtn').onclick = () => {
        initGame(true, false, false); // false = don't keep score for new game
    };
    ...
    document.getElementById('continueGameBtn').onclick = () => {
        ...
        initGame(player1.isHuman, player2.isHuman, true); // true = keep score when continuing
    };
